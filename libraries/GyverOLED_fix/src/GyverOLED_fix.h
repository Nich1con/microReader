/*
	...
	fixed - версия библиотеки для проекта microReader!!!
*/
...
public:
    ...
    // ============================= СЕРВИС =============================== 
    // инициализация
    bool init(int __attribute__((unused)) sda = 0, int __attribute__((unused)) scl = 0) {
        if (_CONN) {            
            SPI.begin();
            pinMode(_CS, OUTPUT);
            fastWrite(_CS, 1);
	...
	void drawBmpFromRam(int x, int y, uint8_t *frame, int width, int height, uint8_t invert = 0, byte mode = 0){
        _x = 0;
        _y = 0;
        ...
        for (int X = x, countX = 0; X < x+width; X++, countX++) {                        // в пикселях
            ...
            if (_inRange(X, 0, _maxX)) {                                                // мы внутри дисплея по X
                for (int Y = y>>3, countY = 0; Y < shiftY; Y++, countY++) {                // в строках (пикс/8)                     
                    byte data = frame[countY*width+countX] ^ invert;    						// достаём байт
